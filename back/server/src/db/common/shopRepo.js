        ...
        SELECT p.*,
               (SELECT MIN(v.price)
                ...
                WHERE v.product_id = p.id
                  AND v.price > 0) AS min_price,
               (SELECT v.image
                FROM product_variants v
                WHERE v.product_id = p.id
                  AND v.image IS NOT NULL
                ORDER BY CASE v.volume
...
export async function createOrder(order, items) {
    ...
    try {
        ...
        // ШАГ 3: пишем заказ
        const orderSql = `
            INSERT INTO orders (customer_name, email, phone, delivery_method, delivery_company, delivery_type,
                                delivery_price, address, pochta_tariff_id, pochta_tariff_name, cdek_tariff_id,
                                cdek_tariff_name, total_price, comment)
